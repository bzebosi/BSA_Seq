  ...
  if(!is_histogram){
    ...
    if (is_smooth) {
      ...
      plot <- plot + geom_point(size = 0.5) + 
        geom_line(aes(y = rollmedian(!!sym(column), rollmedian, na.pad = TRUE)), color = "black", linewidth = 15)
    } else {
      plot <- plot + geom_point(size = 2)
    }
    
    # Add horizontal threshold line if applicable
    if (!is.null(threshold)) {plot <- plot + geom_hline(yintercept = threshold, linetype = "dashed", color = "black", linewidth = 15)}
