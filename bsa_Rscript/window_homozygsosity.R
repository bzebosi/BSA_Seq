...
window_homozygosity <- function(data, af_col = NULL, window_size = 5e5, step_size = 1e5, rollmedian = 21L, nn_prop = 0.1, af_min = 0.99, 
  ...
  if (isTRUE(find_intervals)) {
    ...
    # ---- SAVE intervals to Excel in output_dir ----
    if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)
    excel_file <- file.path(
      output_dir,
      paste0(prefix, "_intervals_", af_col, ".xlsx")
    )
    if (requireNamespace("writexl", quietly = TRUE)) {
      writexl::write_xlsx(list(intervals = intervals), path = excel_file)
      message("Saved intervals to: ", excel_file)
    } else {
      message("Package 'writexl' not installed; skipping Excel export. Install via install.packages('writexl').")
    }
