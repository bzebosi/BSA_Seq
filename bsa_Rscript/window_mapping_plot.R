window_mapping_plot <- function(data, prefix, mt = "mutant", wt = "wildtype", only_mutant = FALSE, use_ems = FALSE,
      ...
      for (tr in tracks) {
        ...
        out[[length(out)+1L]] <- list(column = col, data = df, y_title = ylab,
          plot_title = sprintf("%s | AF | %s", who, 
                               switch(tr, wmd = " windowmedian", lft = "locfit", rmd = "rollingmedian", tr)),
          plotid = sprintf("%s_af_%s", who,
                               switch(tr, wmd = "wmd", lft = "locfit", rmd = "rollmedian", tr))
        )
      }
    }
    out
    ...
    for (m in joint_keys) {
      ...
      for (tr in tracks) {
        ...
        out[[length(out) + 1L]] <- list(column = col, data = df, y_title = ylab,
          ...
          plotid = sprintf("%s_%s_%s_vs_%s", m, 
                           switch(tr, wmd = "wmd", lft = "lft", rmd = "rmd", tr), wt, mt)
        )
      }
    }
    out
  ...
  for (p in params) {
    g <- make_line_plot(
      df_line = p$data, column = p$column, plot_title = p$plot_title, y_title = p$y_title, inbred = inbred,
      facet_layer = facet_layer, color_panel = color_panel, plots_dir = plots_dir, device = device, plot_style = plot_style,
      hwidth = hwidth, hheight = hheight, width = width, height = height, dpi = dpi, file_suffix = p$plotid
    )
