window_bsa_auto <- function(
    ...
    device = "png", dpi = 300,
    plot_style = c("wrap","grid"),
    color_panel = c("blue","red"),
    hwidth = 30, hheight = 18, width = 45, height = 15,
    facet_column = 5, line_size = 5,
    window_size = 5e5, step_size = 1e5, rollmedian = 501L, nn_prop = 0.1,
  ...
  for (pfx in prefix_list) {
    ...
    if (only_mutant) {
      for (mt in mt_list) {
        ...
        results[[key]] <- window_bsa_pipeline(
          vcf_dir = vcf_dir, prefix = pfx, pattern = pattern,
          Genotypes = list(mt = mt),
          output_dir = sub_output,
          only_mutant = TRUE, interval_mutant = TRUE,
          ...
          use_cols = use_cols, do_plot = do_plot, plots_dir = sub_plots,
      ...
      for (wt in wt_list) for (mt in mt_list) {
        ...
        results[[key]] <- window_bsa_pipeline(
          ...
          output_dir = sub_output,
          only_mutant = FALSE, interval_mutant = FALSE,
          use_ems = use_ems, save_results = save_results, save_interval = save_interval,
          rollmedian = rollmedian, window_size = window_size, step_size = step_size,
          nn_prop = nn_prop, find_intervals = find_intervals, offhold = offhold, min_vsize = min_vsize,
          use_cols = use_cols, do_plot = do_plot, plots_dir = sub_plots,
