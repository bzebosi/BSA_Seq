...
window_mapping_compute <- function(data, metric_col = NULL, window_size = 2e6, step_size = 1e5, rollmedian = 100L, nn_prop = 0.1,
                               find_intervals = TRUE, offhold = 0.80, min_vsize = 1e6, use_col = c()){
  ...
  if (find_intervals) {
    ...
    intervals <- peak_interval(data=out, offhold = offhold, min_vsize= min_vsize, use_col = use_col_final)
